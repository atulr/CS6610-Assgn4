...
void glui_button(int control){
	switch(control){
        ...
        case RELOAD_SHADER:
            ...
            cleanupShader(glsl_program_test, vertexShader_test, fragmentShader_test);
...
void initGLUI(){
	...
	// quit button
    glui->add_button("Quit", CB_EXIT, glui_button);
    ...
    // color option
	glui->add_statictext("");
	glui->add_checkbox("Color",&live_optionValue);
    ...
    // empty space
	glui->add_statictext("");
    glui->add_button("Reload Shader", RELOAD_SHADER, glui_button);
 ...
 // upload uniform variable information to the GLSL program
